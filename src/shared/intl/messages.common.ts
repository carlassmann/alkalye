...
let baseCommonMessages = messages({
	...
	"pwa.updateMore":
		".input {$count :number} .match $count one {{and 1 more change}} * {{and {$count} more changes}}",
...
let deCommonMessages = translate(baseCommonMessages, {
	...
	"pwa.updateMore":
		".input {$count :number} .match $count one {{und 1 weitere Änderung}} * {{und {$count} weitere Änderungen}}",
