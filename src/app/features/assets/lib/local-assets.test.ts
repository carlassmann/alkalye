...
describe("local asset files", () => {
	...
	it("detects when a sibling Markdown file still uses an asset", async () => {
		...
		workDirectory.addFile("shared.markdown", `![Map](assets/${id})`)
		expect(await isLocalAssetReferencedElsewhere(location, id)).toBe(true)
		workDirectory.addFile("shared.markdown", "![Different](assets/map-2.png)")
		workDirectory.addFile("shared.txt", `![Map](assets/${id})`)
		expect(await isLocalAssetReferencedElsewhere(location, id)).toBe(true)
		workDirectory.addFile("shared.txt", "![Different](assets/map-2.png)")
